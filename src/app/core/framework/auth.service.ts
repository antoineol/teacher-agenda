...
// TODO use firebase
console.log('TODO use firebase:', firebase);
...
export class AuthService {
	...
	constructor(private authState: AngularFireAuth, @Inject(FirebaseApp) private app:App, private af:AngularFire, private toaster:Toaster) {
		...
		// this.authState.subscribe((authInfo:FirebaseAuthState) => {
	...
	resetPasswordFirebase(email:string):Promise<any> {
		// TODO implement the access view on demand to reset the password
		return <Promise<any>>this.fbAuth.sendPasswordResetEmail(email);
	...
	changePasswordFirebase(newPassword:string/*credentials:FirebaseChangePasswordCredentials*/):Promise<any> {
		console.log("## Don't forget to check the reset password code, if any!");
		return <Promise<any>>this.fbAuth.currentUser.updatePassword(newPassword);
	...
	changeEmailFirebase(newEmail:string):Promise<any> {
		// TODO implement the view for that
		return <Promise<any>>this.fbAuth.currentUser.updateEmail(newEmail);
	...
	ensureAuth():Promise<FirebaseAuthState> {
		return this.authState.first().toPromise().then((authInfo:FirebaseAuthState) => {
	...
	login(options?:AuthConfiguration, credentials?:Credentials):Promise<any> {
		let loginPromise = credentials ? this.af.auth.login(credentials, options) : this.af.auth.login(options);
		return <Promise<any>>loginPromise.then((user:FirebaseAuthState) => {
	...
	signup(credentials:EmailPasswordCredentials):Promise<any> {
		credentials.password = Utils.randomPassword();
		// angularfire2 beta 2
		return <Promise<any>>this.af.auth.createUser(credentials).then((authData:FirebaseAuthState) => {
	...
	changePassword(newPassword:string /*credentials:FirebaseChangePasswordCredentials*/):Promise<any> {
